...
- **Test functions and methods invert the imperative-mood rule.** They MUST be named `test_*` (pytest's collection contract). Their docstrings MUST state a truth-apt proposition in the present-tense indicative -- the falsifiable claim the test body exists to refute -- not an imperative verb phrase. `Identical flat dicts score 1.0.`, `Unknown nested keys are rejected.` (passive), and `The schema rejects unknown nested keys.` (active) are all correct; `Reject unknown nested keys.` (imperative) and `Assert that identical flat dicts score 1.0.` (framing verb) are not. Active and passive voice are equally acceptable -- pick whichever reads as the clearest claim; a trailing `..., so <why it matters>` clause is OPTIONAL. This inversion covers test function and method docstrings only -- module docstrings in test files follow the imperative rule above by convention, since ruff `D401` only ever checks functions and methods. `tests/**/test_*.py` is exempt from `D401` so declarative first lines pass lint; because per-file ignores are file-scoped, that exemption also lifts `D401` from non-test helper functions defined in those files. Dedicated helper modules (`conftest.py`, `_*.py`) keep imperative-mood enforcement.
