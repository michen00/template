...
# That is benign only when git cliff found nothing to release: cliff.toml always
# emits the header/footer boilerplate, so the output is never byte-empty, and
# when the tip carries a moving alias tag (v1) beside the version tag git cliff
# renders an empty "## [1.2.3]" heading instead of "## [Unreleased]". Byte
# emptiness is therefore not a usable test. But if the output does carry a
# rendered release body -- '--cliff-args -- --tag vX.Y.Z', or a custom --config
# whose template uses a different heading -- then the extractor simply could not
# find it, and exiting 0 would leave ${CHANGELOG} silently stale.
cliff_output_has_release_body() {
  awk '
    /^<!-- generated by git-cliff -->/ { exit }
    # Entry markup proves a rendered body whatever heading sits above it. A
    # custom --config template may emit any heading level, so keying only off
    # "## " would miss e.g. "# Next release" followed by commit bullets.
    /^(### |#### |- |\* |\+ )/ { content = 1; exit }
    /^## / { seen_heading = 1; next }
    seen_heading && NF { content = 1; exit }
    END { exit (content ? 0 : 1) }
  ' "$1"
}

if [ ! -s "$TEMP_FILE" ]; then
  if cliff_output_has_release_body "$CLIFF_OUTPUT"; then
    echo "Error: git cliff produced changelog content with no '## [Unreleased]' heading." >&2
    echo "The generated section cannot be applied to ${CHANGELOG}." >&2
    echo "Check --cliff-args (e.g. --tag pins a versioned heading) and the template." >&2
    exit 1
  fi
