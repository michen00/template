...
# Emits a mock git-cliff whose output is the given heredoc on stdin. Used by the
# three empty-extraction cases below, where what matters is the exact shape of
# the output rather than a realistic Unreleased section.
setup_mock_cliff_with_output() {
  local test_name="$1"
  local cliff_output="$TEST_ROOT/$test_name/mock-cliff-output.md"
  local bin_dir="$TEST_ROOT/$test_name/bin"
  mkdir -p "$bin_dir"
...
# The next four tests cover the empty-extraction guard. The extraction is empty
# exactly when git cliff's output carries no "## [Unreleased]" heading, and the
# guard asks git whether that is expected: benign when HEAD sits on a release
# tag, a misconfiguration otherwise. The output's shape is deliberately not
# consulted, so the pairs below vary tag state while holding shape roughly
# constant, and vice versa.
test_empty_cliff_output_on_release_tag_is_noop() {
  local repo bin_dir out
  repo="$(setup_test_repo "empty-on-tag")"
  write_default_changelog "$repo/CHANGELOG.md"
  commit_file "$repo" "CHANGELOG.md" "test: add changelog fixture"
  (cd "$repo" && git tag v1.0.2)
  bin_dir="$(setup_mock_cliff_with_output "empty-on-tag" < /dev/null)"

  (
    cd "$repo"
    out="$(PATH="$bin_dir:$PATH" MOCK_CLIFF_OUTPUT="$TEST_ROOT/empty-on-tag/mock-cliff-output.md" "$SUT" 2>&1)"
    [[ "$out" == *"No unreleased changes found"* ]] || fail "Expected no-op message, got: $out"
    git diff --quiet HEAD -- CHANGELOG.md || fail "Expected no changelog changes"
  )
}

# The shape the real 2026-06-09 CI failure produced: a moving alias tag beside
# the version tag makes git cliff render an empty "## [1.0.2]" heading instead of
# "## [Unreleased]". HEAD is on a tag, so there is nothing to apply and this must
# stay a clean no-op. This is the regression guard that matters most.
test_versioned_heading_on_release_tag_is_noop() {
  local repo bin_dir out
  repo="$(setup_test_repo "versioned-on-tag")"
  write_default_changelog "$repo/CHANGELOG.md"
  commit_file "$repo" "CHANGELOG.md" "test: add changelog fixture"
  # Both tags on one commit is what collided git-cliff's release segmentation.
  (cd "$repo" && git tag v1.0.2 && git tag v1)
  bin_dir="$(
    setup_mock_cliff_with_output "versioned-on-tag" << 'EOF'
# Changelog
...
EOF
  ...
  (
    cd "$repo"
    out="$(PATH="$bin_dir:$PATH" MOCK_CLIFF_OUTPUT="$TEST_ROOT/versioned-on-tag/mock-cliff-output.md" "$SUT" 2>&1)"
    [[ "$out" == *"No unreleased changes found"* ]] || fail "Expected no-op message, got: $out"
    git diff --quiet HEAD -- CHANGELOG.md || fail "Expected no changelog changes"
  )
  assert_file_contains "$repo/CHANGELOG.md" "- old feature placeholder"
}

# Same output shape as the no-op above, but HEAD has moved past the tag. Commits
# exist that should have been rendered under "## [Unreleased]", so the config is
# incompatible -- what '--cliff-args -- --tag vX.Y.Z' produces -- and exiting 0
# would leave the changelog silently stale.
test_missing_unreleased_heading_off_tag_fails() {
  local repo bin_dir out
  repo="$(setup_test_repo "missing-off-tag")"
  write_default_changelog "$repo/CHANGELOG.md"
  commit_file "$repo" "CHANGELOG.md" "test: add changelog fixture"
  (cd "$repo" && git tag v1.0.2)
  printf 'x\n' > "$repo/feature.txt"
  commit_file "$repo" "feature.txt" "feat: land something after the tag"
  bin_dir="$(
    setup_mock_cliff_with_output "missing-off-tag" << 'EOF'
# Changelog
...
- folded into a versioned heading instead of Unreleased

<!-- generated by git-cliff -->
EOF
  )"

  if out="$(cd "$repo" && PATH="$bin_dir:$PATH" MOCK_CLIFF_OUTPUT="$TEST_ROOT/missing-off-tag/mock-cliff-output.md" "$SUT" 2>&1)"; then
    fail "Expected a missing Unreleased heading off-tag to fail"
  fi
  [[ "$out" == *"no '## [Unreleased]' heading"* ]] || fail "Expected missing-heading error, got: $out"
  assert_file_contains "$repo/CHANGELOG.md" "- old feature placeholder"
}

# Markup the guard never enumerates: a numbered list under a level-one heading.
# Because the decision comes from git rather than the output, heading level and
# list style are irrelevant and this fails for the same reason as the test above.
test_unrecognized_markup_off_tag_fails() {
  local repo bin_dir out
  repo="$(setup_test_repo "markup-off-tag")"
  write_default_changelog "$repo/CHANGELOG.md"
  commit_file "$repo" "CHANGELOG.md" "test: add changelog fixture"
  (cd "$repo" && git tag v1.0.2)
  printf 'x\n' > "$repo/feature.txt"
  commit_file "$repo" "feature.txt" "feat: land something after the tag"
  bin_dir="$(
    setup_mock_cliff_with_output "markup-off-tag" << 'EOF'
# Changelog
...
EOF
  ...
  if out="$(cd "$repo" && PATH="$bin_dir:$PATH" MOCK_CLIFF_OUTPUT="$TEST_ROOT/markup-off-tag/mock-cliff-output.md" "$SUT" 2>&1)"; then
    fail "Expected unrecognized markup off-tag to fail"
  fi
  [[ "$out" == *"no '## [Unreleased]' heading"* ]] || fail "Expected missing-heading error, got: $out"
  assert_file_contains "$repo/CHANGELOG.md" "- old feature placeholder"
}
