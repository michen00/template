...
# The next three tests cover the empty-extraction guard. The extraction is empty
# exactly when git cliff's output carries no "## [Unreleased]" heading, which is
# benign for a release with nothing to report but a real misconfiguration when
# actual content was rendered under some other heading.
test_empty_cliff_output_is_noop() {
  local repo bin_dir out
  repo="$(setup_test_repo "empty-cliff-output")"
  write_default_changelog "$repo/CHANGELOG.md"
  commit_file "$repo" "CHANGELOG.md" "test: add changelog fixture"
  bin_dir="$(setup_mock_cliff_with_output "empty-cliff-output" < /dev/null)"

  (
    cd "$repo"
    out="$(PATH="$bin_dir:$PATH" MOCK_CLIFF_OUTPUT="$TEST_ROOT/empty-cliff-output/mock-cliff-output.md" "$SUT" 2>&1)"
...
# "## [Unreleased]". There is nothing to apply, so this must stay a clean no-op.
test_versioned_heading_without_body_is_noop() {
  local repo bin_dir out
  repo="$(setup_test_repo "versioned-empty")"
  write_default_changelog "$repo/CHANGELOG.md"
  commit_file "$repo" "CHANGELOG.md" "test: add changelog fixture"
  bin_dir="$(
    setup_mock_cliff_with_output "versioned-empty" << 'EOF'
# Changelog
...
EOF
  ...
  (
    cd "$repo"
    out="$(PATH="$bin_dir:$PATH" MOCK_CLIFF_OUTPUT="$TEST_ROOT/versioned-empty/mock-cliff-output.md" "$SUT" 2>&1)"
    [[ "$out" == *"No unreleased changes found"* ]] || fail "Expected no-op message, got: $out"
    git diff --quiet HEAD -- CHANGELOG.md || fail "Expected no changelog changes"
  )
...
# Real rendered content under a heading the extractor cannot match -- what
# '--cliff-args -- --tag vX.Y.Z' or a custom template produces. Exiting 0 here
# would leave the changelog silently stale, so this must fail loudly.
test_rendered_body_without_unreleased_heading_fails() {
  local repo bin_dir out
  repo="$(setup_test_repo "body-no-unreleased")"
  write_default_changelog "$repo/CHANGELOG.md"
  commit_file "$repo" "CHANGELOG.md" "test: add changelog fixture"
  bin_dir="$(
    setup_mock_cliff_with_output "body-no-unreleased" << 'EOF'
# Changelog
...
EOF
  ...
  if out="$(cd "$repo" && PATH="$bin_dir:$PATH" MOCK_CLIFF_OUTPUT="$TEST_ROOT/body-no-unreleased/mock-cliff-output.md" "$SUT" 2>&1)"; then
    fail "Expected rendered content with no Unreleased heading to fail"
  fi
  [[ "$out" == *"no '## [Unreleased]' heading"* ]] || fail "Expected missing-heading error, got: $out"
  assert_file_contains "$repo/CHANGELOG.md" "- old feature placeholder"
}

# A custom --config template can render entries under any heading level, so the
# body check cannot key only off "## ". Bullets under a level-one heading are
# still a rendered body the extractor failed to find.
test_rendered_body_under_other_heading_level_fails() {
  local repo bin_dir out
  repo="$(setup_test_repo "body-other-level")"
  write_default_changelog "$repo/CHANGELOG.md"
  commit_file "$repo" "CHANGELOG.md" "test: add changelog fixture"
  bin_dir="$(
    setup_mock_cliff_with_output "body-other-level" << 'EOF'
# Changelog

# Next release

- rendered under a level-one heading

<!-- generated by git-cliff -->
EOF
  )"

  if out="$(cd "$repo" && PATH="$bin_dir:$PATH" MOCK_CLIFF_OUTPUT="$TEST_ROOT/body-other-level/mock-cliff-output.md" "$SUT" 2>&1)"; then
    fail "Expected rendered content under a non-level-two heading to fail"
  fi
  [[ "$out" == *"no '## [Unreleased]' heading"* ]] || fail "Expected missing-heading error, got: $out"
  assert_file_contains "$repo/CHANGELOG.md" "- old feature placeholder"
}
