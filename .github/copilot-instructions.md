...
- **CI Expectations:** `.github/workflows/CI.yml` runs `nox -s test` and `nox -s precommit`. Note: this is similar to `make check`—the latter also runs ruff formatting via `format-unsafe`, which is not included in the CI workflow. `.github/workflows/.cache-buster` lets us reset GitHub cache via `make bust-ci-cache`—keep it in the manifest.
