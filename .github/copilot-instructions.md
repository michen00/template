...
- **CI Expectations:** `.github/workflows/CI.yml` runs `nox -s test` and `nox -s precommit`. Before running precommit, CI executes `.github/scripts/ci-ruff-args.sh` to modify `.pre-commit-config.yaml` with stricter ruff args and GitHub-optimized output formatting. `make check` now mirrors CI's local flow more closely by running `make tidy` plus both pre-commit hook stages before tests; the main remaining difference is that local checks use the default ruff args from `.pre-commit-config.yaml`. `.github/workflows/.cache-buster` lets us reset GitHub cache via `make bust-ci-cache`—keep it in the manifest.
