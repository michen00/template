...
- **CI Expectations:** `.github/workflows/CI.yml` runs `nox -s test` and `nox -s precommit` (equivalent to `make check`). `.github/workflows/.cache-buster` lets us reset GitHub cache via `make bust-ci-cache`—keep it in the manifest.
