...
`.gitignore` is generated by `scripts/concat-gitignores.sh`; regenerate it with `bash scripts/concat-gitignores.sh` rather than editing it by hand.

- **Rule:** a new ignore pattern goes in `scripts/custom.gitignore`, which is appended verbatim to the end of the generated file. Only patterns that would be correct in **every** repository belong in the script's own tail — that is what lets the script be synced between repositories unchanged.
- The custom file is optional; the script reports it and carries on when absent.
...
- When making significant changes to the template, keep template-scope guidance internally consistent:
...
- When shared behavior changes (CI, Makefile targets, coding standards, setup expectations), review derived-project counterparts (`.AGENTS.md`, `.CLAUDE.md`, `.github/.copilot-instructions.md`, `.specify/memory/.constitution.md`) and update as needed. These files may intentionally diverge from template-scope guidance.
