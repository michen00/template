...
`.gitignore` is generated by `scripts/concat-gitignores.sh`; regenerate it with
`bash scripts/concat-gitignores.sh` rather than editing it by hand.

- **Rule:** a new ignore pattern goes in `scripts/custom.gitignore`, which is
  appended verbatim to the end of the generated file. Only patterns that would be
  correct in **every** repository belong in the script's own tail — that is what
